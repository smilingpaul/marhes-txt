/**
 ...
 @brief Includes functions to initialize the LCD's SPI communication and the LCD
        itself. Also has functions to write low and high level objects to the 
        display.
        
 This code is based on Jim Lynch's driver.
...
/// Small (6 by 8 pixel) character array
...
/// Medium (8 by 8 pixel) character array
...
/// Large (8 by 16 pixel) character array
...
/**
 @brief Initialize the SPI to talk with the LCD, then initialize the LCD.
 The SPI clock is set at around 1MHz. The LCD doesn't work with >~5MHz clocks. 
*/
void LcdInit(void)
{
  volatile uint32_t j;
...
/**
 @brief Sends a command to the LCD over SPI
 
 To send a command, bit 8 is cleared
 @param[in] command The LCD command to be sent
*/
...
/**
 @brief Sends a byte of data to the LCD over SPI
 
 To send data, bit 8 is set
 @param[in] data The LCD data byte to be sent
*/
void LcdSendData(unsigned char data)
{
  ...
  SSP0DR = output;                          // Send data
}

/**
 @brief Turns on and off the LCD Backlight
 @param[in] state ON = 1, OFF = 0
 @note Not used because PWM controls the brightness
*/
...
/**
 ...
 @param[in] color The color to write to the LCD from the 12-bit color
                  definitions in lcd.h:107
*/
...
/**
 @brief Sets the cursor to the XY position on the LCD
 @param[in] x The address of the row
 @param[in] y The address of the column
*/
...
/**
 @brief Sets the pixel to the color at the X, Y coordinate.
 @param[in] x The address of the row
 @param[in] y The address of the column
 @param[in] color The color to write to the LCD from the 12-bit color
                  definitions in lcd.h:107
...
//          LcdSetLine.c
//
//  Draws a line in the specified color from (x0,yStart) to (x1,y1)
//
// Inputs:    x     =   row address (0 .. 131)
//      y     =   column address  (0 .. 131)
//      color =   12-bit color value  rrrrggggbbbb
...
/**
 ...
 @param[in] xStart The starting address of the row
 @param[in] yStart The starting address of the column
 @param[in] xEnd   The ending address of the row
 @param[in] yEnd   The ending address of the column
 @param[in] color  The color to write to the LCD from the 12-bit color
                   definitions in lcd.h:107
*/
void LcdSetLine(int xStart, int yStart, int xEnd, int yEnd, int color) {
  ...
  if (dy < 0) { dy = -dy;  stepy = -1; } else { stepy = 1; }
    ...
    LcdSetPixel(xStart, yStart, color);
...
/**
 @brief Draws a rectangle from two coordinates
 @param[in] xStart The starting address of the row
 @param[in] yStart The starting address of the column
 @param[in] xEnd   The ending address of the row
 @param[in] yEnd   The ending address of the column
 @param[in] fill   Fill = 1, No Fill = 0
 @param[in] color  The color to write to the LCD from the 12-bit color
                   definitions in lcd.h:107
*/
void LcdSetRect(int xStart, int yStart, int xEnd, int yEnd, unsigned char fill, int color) {
  ...
  if (fill == FILL) {
    ...
    xmin = (xStart <= xEnd) ? xStart : xEnd;
    xmax = (xStart > xEnd) ? xStart : xEnd;
    ymin = (yStart <= yEnd) ? yStart : yEnd;
    ...
    LcdSetLine(xStart, yStart, xEnd, yStart, color);
    LcdSetLine(xStart, yEnd, xEnd, yEnd, color);
    LcdSetLine(xStart, yStart, xStart, yEnd, color);
    LcdSetLine(xEnd, yStart, xEnd, yEnd, color);
  }
}
...
/**
 @brief Draws a circle at the specified coordinate and radius
 @param[in] xStart The center address of the row
 @param[in] yStart The center address of the column
...
void LcdSetCircle(int xStart, int yStart, int radius, int color) {
  ...
  LcdSetPixel(xStart, yStart + radius, color);
  LcdSetPixel(xStart, yStart - radius, color);
  LcdSetPixel(xStart + radius, yStart, color);
  LcdSetPixel(xStart - radius, yStart, color);
  ...
  while (x < y) {
    ...
    ddF_x += 2;
    f += ddF_x + 1;
    LcdSetPixel(xStart + x, yStart + y, color);
    LcdSetPixel(xStart - x, yStart + y, color);
    LcdSetPixel(xStart + x, yStart - y, color);
    LcdSetPixel(xStart - x, yStart - y, color);
    LcdSetPixel(xStart + y, yStart + x, color);
    LcdSetPixel(xStart - y, yStart + x, color);
    LcdSetPixel(xStart + y, yStart - x, color);
    LcdSetPixel(xStart - y, yStart - x, color);
  }
...
/**
 @brief Draws a character at the address
 @param[in] c      The character to be written 
 @param[in] x      The center address of the row
 @param[in] y      The center address of the column
 @param[in] size   The size of the character, 0 = Small, 1 = Medium, 2 = Large
 @param[in] fcolor The foreground color to write to the LCD from the 12-bit 
                   color definitions in lcd.h:107
 @param[in] bcolor The background color to write to the LCD from the 12-bit
                   color definitions in lcd.h:107
*/
...
/**
 @brief Draws an array of character at the address
 @param[in] c      The null terminated string to be written 
 @param[in] x      The center address of the row
 @param[in] y      The center address of the column
 @param[in] size   The size of the character, 0 = Small, 1 = Medium, 2 = Large
 @param[in] fcolor The foreground color to write to the LCD from the 12-bit 
